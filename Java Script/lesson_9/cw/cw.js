...
simpsons.forEach(value => {
    ...
    for (const valueKey in value) {
        if (valueKey !== 'photo') {
            ...
            smallDiv.innerText = `${valueKey}:${value[valueKey]}`;
            div.appendChild(smallDiv);
        } else {
            let img = document.createElement('img');
            img.src = `${value[valueKey]}`;
            div.appendChild(img);
        }
    }
    document.body.appendChild(div);
...
// в яких будуть зберігатись значення окремих властивостей, для властивості
// modules зробити список з елементами
// Приклад структири знаходиться у файлі example.png

coursesArray.forEach(value => {
    let valueDiv = document.createElement('div');
    valueDiv.classList.add('revision');
    ...
    moduleDiv.classList.add('model');

    titleDiv.innerText = value.title;
    monthDurationDiv.innerText = value.monthDuration;
    hourDurationDiv.innerText = value.hourDuration;
    value.modules.forEach(modelElement => {
        let modelElementDiv = document.createElement('div');
        modelElementDiv.innerText = modelElement;
        moduleDiv.appendChild(modelElementDiv);


    });
    valueDiv.append(titleDiv, DurationDiv, moduleDiv);
